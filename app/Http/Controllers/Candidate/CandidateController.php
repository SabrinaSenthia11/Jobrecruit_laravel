...
class CandidateController extends Controller
{
    public function dashboard()
    {
        ...
        $total_applied_jobs = CandidateApplication::where('candidate_id',FacadesAuth::guard('candidate')->user()->id)->where('status','Applied')->count();

        $total_rejected_jobs = CandidateApplication::where('candidate_id',FacadesAuth::guard('candidate')->user()->id)->where('status','Rejected')->count();

        $total_approved_jobs = CandidateApplication::where('candidate_id',FacadesAuth::guard('candidate')->user()->id)->where('status','Approved')->count();
     
        $total_notification = CandidateApplication::where('candidate_id',FacadesAuth::guard('candidate')->user()->id)->where('status','message')->count();


        return view('candidate.dashboard',compact('total_applied_jobs','total_rejected_jobs','total_approved_jobs','total_notification'));
    }
    ...
    {
        $obj = Candidate::where('id',FacadesAuth::guard('candidate')->user()->id)->first();
        $id = $obj->id;
        ...
        if($request->hasFile('photo')) {
            ...
            if(FacadesAuth::guard('candidate')->user()->photo != '') {
    ...
    public function edit_password_update(Request $request)
    {
        ...
        $obj = Candidate::where('id',FacadesAuth::guard('candidate')->user()->id)->first();
        $obj->password = FacadesHash::make($request->password);
        $obj->update();
    ...
    public function education()
                                                                      // 118line after guadr placing the ('guardName') will not work the education view
    {
        $educations = CandidateEducation::where('candidate_id', FacadesAuth::guard('candidate')->user()->id)->orderBy('id','desc')->get();
    ...
    public function education_store(Request $request)
    {
        ...
        $obj->candidate_id = FacadesAuth::guard('candidate')->user()->id;
    ...
    {
        $skills = CandidateSkill::where('candidate_id',FacadesAuth::guard('candidate')->user()->id)->get();
    ...
    {
        $experiences = CandidateExperience::where('candidate_id',FacadesAuth::guard('candidate')->user()->id)->orderBy('id','desc')->get();
    ...
    {
        $awards = CandidateAward::where('candidate_id',FacadesAuth::guard('candidate')->user()->id)->orderBy('id','desc')->get();
    ...
    {
        $resumes = CandidateResume::where('candidate_id',FacadesAuth::guard('candidate')->user()->id)->get();
    ...
    {
        $existing_bookmark_check = CandidateBookmark::where('candidate_id', FacadesAuth::guard('candidate')->user()->id)->where('job_id',$id)->count();
    ...
    {
        $bookmarked_jobs = CandidateBookmark::with('rJob','rCandidate')->where('candidate_id',FacadesAuth::guard('candidate')->user()->id)->get();
    ...
    {
        $existing_apply_check = CandidateApplication::where('candidate_id',FacadesAuth::guard('candidate')->user()->id)->where('job_id',$id)->count();
    ...
    {
        $applied_jobs = CandidateApplication::with('rJob')->where('candidate_id',FacadesAuth::guard('candidate')->user()->id)->get();
        return view('candidate.applications', compact('applied_jobs'));
    }

    public function Interviews()
    {
        $interview = candidateInterview::where('candidate_id', FacadesAuth::user()->id)->first();
        if (!$interview) {
            // Interview not found, redirect to a different page
            return redirect()->back()->with('error', 'You do not have any interview message!');

        }
    
        $job = Job::where('id', $interview->job_id)->first();
        return view('candidate.job_notice', compact('interview','job'));
    }
}
